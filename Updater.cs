        ...
        protected Status DoDownload(ApplicationJob job, Uri urlToRequest)
        {
            ...
            using (WebResponse response = WebClient.GetResponse(req))
            {
                ...
                if (segmentCount > 1)
                {
                    ...
                    Downloader d = new Downloader(new ResourceLocation { Url = urlToRequest.AbsoluteUri, ProtocolProvider = new GeGeekProtocolProvider(job, m_Cookies) }, null, tmpLocation, segmentCount);
                    d.Start();
        ...
        private static long GetContentLength(WebResponse response)
        {
            ...
            if (ftp != null)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ftpConnection = new FTP(response.ResponseUri.Host, "anonymous", "GeGeek@canneverbe.com");
